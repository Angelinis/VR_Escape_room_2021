...
namespace UnityEngine.XR.Interaction.Toolkit
{
    ...
    public abstract class EditedSnapTurnProviderBase : LocomotionProvider
    {
        [SerializeField]
        ...
        [SerializeField]
        private AudioClip northClip;
        [SerializeField]
        private AudioClip southClip;
        [SerializeField]
        private AudioClip westClip;
        [SerializeField]
        private AudioClip eastClip;
        ...
        protected void StartTurn(float amount)
        {
            ...
            if (Time.time - lastStepTime > stepSoundInterval)
            {
                ...
                var xrOriginRotationY = NormalizeEulerAngle(system.xrOrigin.transform.rotation.eulerAngles.y + m_CurrentTurnAmount);
                //float totalRotation = xrOriginRotationY + m_CurrentTurnAmount;
                int finalVal = Mathf.RoundToInt(xrOriginRotationY);
                //int finalVal = Mathf.RoundToInt(totalRotation);
                switch (finalVal)
                {
                    case 0:
                        if (northClip)
                            rotationAudioSource.clip = northClip;
                        rotationAudioSource.Play();
                        //Debug.Log("North");
                        break;
                    case 90:
                        if (eastClip)
                            rotationAudioSource.clip = eastClip;
                        rotationAudioSource.Play();
                        //Debug.Log("East");
                        break;
                    case 180:
                        if (southClip)
                            rotationAudioSource.clip = southClip;
                        rotationAudioSource.Play();
                        //Debug.Log("South");
                        break;
                    case -180:
                    ...
                    case -90:
                        if (westClip)
                            rotationAudioSource.clip = westClip;
                        rotationAudioSource.Play();
                        //Debug.Log("West");
                        break;
                }
